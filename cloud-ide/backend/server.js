...
// const { spawn } = require('child_process'); // NEW: For running the build process
...
// // Notice we added `req` so we can read the URL parameters!
// wss.on('connection', async (ws, req) => {
//   // Parse the requested environment from the connection URL
//   const url = new URL(req.url, `http://${req.headers.host}`);
//   const envName = url.searchParams.get('env');
  
//   // Create a safe, lowercase image name for Docker
//   const safeImageName = envName ? `custom-ide-${envName.toLowerCase().replace(/[^a-z0-9]/g, '-')}` : 'ubuntu:latest';

//   let ptyProcess = null;

//   try {
//     // IF WE HAVE A CUSTOM ENVIRONMENT, BUILD IT FIRST
...
//         // Spawn the Docker Build process
//         await new Promise((resolve, reject) => {
//           const buildProcess = spawn('docker', ['build', '-t', safeImageName, '.'], { cwd: buildDir });
          
//           // Stream build logs directly to the user's browser terminal!
...
//     // --- SPAWN THE INTERACTIVE TERMINAL ---
//     ptyProcess = pty.spawn('docker', [
//       'run', '-it', '--rm', 
//       '-e', 'TERM=xterm-256color', // 🚨 THE FIX: Tell Ubuntu our exact terminal type!
//       safeImageName, 
//       'bash'
//     ], {
//       name: 'xterm-256color', // Match the name here
//       cols: 80,
//       rows: 24,
//     });

//     // Handle incoming WebSocket messages from React
//     ws.on('message', (message) => {
//       // Convert the raw binary buffer into a string safely
//       const msgStr = message.toString();
      
//       // Check if it is our custom resize JSON from the frontend
...
//         } catch (e) {}
//       } else {
//         // Otherwise, pipe the raw keystrokes straight to Docker
...
//   // Allow the frontend to resize the Docker PTY
//     ws.on('message', (message) => {
//       const msgStr = message.toString();
//       // Intercept a custom resize JSON payload
//       if (msgStr.startsWith('{"type":"resize"')) {
//         const { cols, rows } = JSON.parse(msgStr);
//         if (ptyProcess) ptyProcess.resize(cols, rows);
//       } else {
//         // Otherwise, pipe standard typing to Docker
//         if (ptyProcess) ptyProcess.write(message);
//       }
//     });
// });
const express = require('express');
