...
| **O4** ◑ | **Tune Xvnc + noVNC encoding** | **Client half DONE:** `DisplayPane` sets `compressionLevel 0` + `qualityLevel 8` — on localhost/LAN bandwidth is surplus and CPU scarce (software-GL server + JS decoder), so we trade bytes for cycles (skip zlib both ends, keep the UI crisp). Client-only, cannot affect Xvnc boot. **Pending:** the bigger lever is server-side frame coalescing (Xvnc `-DeferUpdate`, coalesce a busy update window). It needs a **live Xvnc to verify the flag and tune the value**, and a bad flag would fail the display start — so it waits for the slice-5 E2E gate, per the "be sure it won't break" rule. | Client half: none (client-only). Server flag: contained (a wrong flag only fails `startDisplay`, retryable) but unverified — hence deferred. | Client: shipped. Server: after E2E gate. |
| **O5** ✅ | **Shrink the audio ring** | **DONE.** Worklet ring cut from ~2 s to ~0.5 s (`sampleRate*channels*0.5`) — absorbs localhost/LAN jitter and caps worst-case latency (a bigger ring just accumulates backlog during recovery). Overflow still drops oldest → silence, never a glitch. | — | shipped |
